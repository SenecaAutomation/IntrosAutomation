 ...
 *
 */
public class SenecaThucydidesParameterizedRunner extends Suite {
    ...
    private final SenecaParameterizedTestsOutcomeAggregator senecaParameterizedTestsOutcomeAggregator = SenecaParameterizedTestsOutcomeAggregator.from(this);

    /**
     * Only used for testing.
     */
    public SenecaThucydidesParameterizedRunner(final Class<?> klass,Configuration configuration,final WebDriverFactory webDriverFactory) throws Throwable {
    ...
    private void scheduleParallelTestRunsFor(final Class<?> klass) {
        setScheduler(new SenecaParameterizedRunnerScheduler(klass, getThreadCountFor(klass)));
    }
    ...
    private void buildTestRunnersForEachDataSetUsing(final WebDriverFactory webDriverFactory) throws Throwable {
        ...
        for (int i = 0; i < parametersTable.getRows().size(); i++) {
            ...
            ThucydidesRunner runner = new SenecaTestClassRunnerForParameters(testClass,
                                                                       configuration,
                                                                       webDriverFactory,
                                                                       parametersTable,
                                                                       i);
    ...
    private void buildTestRunnersFromADataSourceUsing(final WebDriverFactory webDriverFactory) throws Throwable {
        ...
        for (int i = 0; i < testCases.size(); i++) {
            ...
            ThucydidesRunner runner = new SenecaTestClassRunnerForInstanciatedTestCase(testCase,configuration,webDriverFactory,parametersTable,i);
     ...
     */
    public SenecaThucydidesParameterizedRunner(final Class<?> klass) throws Throwable {
    ...
    public void generateReports() {
    	...
    	List<TestOutcome> testOutcomes=senecaParameterizedTestsOutcomeAggregator.aggregateTestOutcomesByTestMethods();
    	for(int i=0;i<testOutcomes.size();i++){
    		TestOutcome testOutCome=testOutcomes.get(i);
    		CommonUtil.setTestoutComeTags(testOutCome);
    	}
